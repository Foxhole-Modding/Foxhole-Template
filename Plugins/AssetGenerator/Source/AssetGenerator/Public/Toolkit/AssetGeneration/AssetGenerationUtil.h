...
struct ASSETGENERATOR_API UDeserializedProperty {
public:
	FName PropertyName;
	...
	FORCEINLINE UDeserializedProperty() : PropertyFlags(CPF_None), ArrayDim(1), BlueprintReplicationCondition(COND_None) {}
	
	UDeserializedProperty(const TSharedPtr<FJsonObject>& Object, class UObjectHierarchySerializer* ObjectSerializer);
...
public:
	...
	TArray<UDeserializedProperty> AllProperties;
	TArray<UDeserializedProperty> Parameters;
	UDeserializedProperty ReturnValue;
	...
	FORCEINLINE bool HasAnyOutputParams() const {
		for (const UDeserializedProperty& Param : Parameters) {
