...
#define CHECK_AND_LOG_PARAM(ParamName, TitleText) \
	if (ParamName.Num()) { \
		UE_LOG(LogAssetDumper, Display, TitleText); \
		for (const decltype(ParamName)::ElementType& Element : ParamName) { \
			UE_LOG(LogAssetDumper, Display, TEXT("- %s"), *FAssetTypeSerializerMacros_Internals::ValueToString<decltype(ParamName)::ElementType>(Element)); \
		} \
	}
